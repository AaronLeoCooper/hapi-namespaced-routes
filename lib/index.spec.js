...
test("routes shouldn't be mutated", function() {
  const route = spy()
  ...
  assert.equal(
    routeModule.routes[0].path,
    '',
    "first route shouldn't be mutated"
  )

  assert.equal(
    routeModule.routes[1].path,
    '/path2',
    "second route shouldn't be mutated"
  )
})
